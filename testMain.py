...
# Global variable to track the new window
tool_window = None

#creating a port scanning window with systematic ui
def open_tools_window():
    global tool_window
    # Create a new window
    #logic to check if the windows is already open or not 
    if tool_window is None or not Toplevel.winfo_exists(tool_window):
        tool_window = Toplevel(root)
        tool_window.title("Tools")
        tool_window.geometry("300x300")  # Set dimensions of the new window
        
        # in this section we can add contents of the new window
        # Target IP Label and Entry
        target_ip_label = Label(tool_window, text="Target IP:")
        target_ip_label.grid(row=1, column=0)
        global target_ip_entry
        target_ip_entry = Entry(tool_window, width=18, textvariable=target_for_portscan)
        target_ip_entry.grid(row=1, column=1)

        # Scan Type Drop-down Menu
        scan_type_label = Label(tool_window, text="Scan Type:")
        ...
        port_input_label = Label(tool_window, text="Port(s):")
        port_input_label.grid(row=3, column=0)
        global port_input_box
        port_input_box = Entry(tool_window, width=24)
        port_input_box.grid(row=3, column=1)

        # Protocol Drop-down Menu
        protocol_label = Label(tool_window, text="Protocol:")
        ...
        # Scan Button
        port_scan_button = Button(tool_window, text="Start Scan", command=validation_and_port_scan)
